#ifndef BLOC_RECEIVE_STREAM_H
#define BLOC_RECEIVE_STREAM_H
...
class ReceiveStream {
public:
    using result_type = std::expected<PooledBuffer<Context>, std::error_code>;

    class NextAwaiter {
    public:
        using resume_type = PooledBuffer<Context>;
    ...
    ReceiveStream(Context& context, int fd, unsigned bgid)
      : context_{ &context }, fd_{ fd }, bgid_{ bgid }
    {}
...
private:
    class MutishotReceiveOperation: public Operation {
    public:
        MutishotReceiveOperation(ReceiveStream* stream, Context* context, unsigned bgid)
          : stream_{ stream }, context_{ context }, bgid_{ bgid } 
        {}
...
template<typename Context>
void ReceiveStream<Context>::MutishotReceiveOperation::complete(int res, unsigned flags)
{

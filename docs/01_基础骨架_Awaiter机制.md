### 目标与设计边界
这个项目想做的是一个基于 `io_uring` 封装的 C++ 协程网络库。动手之前，先把设计边界定死：
...
在基础设施级别的系统库中，性能一旦架构确定就基本锁死，后期很难靠"手法"弥补，所以这个基调要在最开始就定好。我们选择不给自己埋雷，直接将底层与 `io_uring` 强绑定。
...
> 只有这个标志位是不够的，这里埋着两个独立的问题。第一个是唤醒：`run()` 阻塞在 `io_uring_submit_and_wait` 时，`should_stop_` 的变化根本不会被看到，内核不会主动返回。这个问题会在下一篇用 `eventfd` 解决。第二个是清理：`run()` 即便退出了，内核里可能还有已提交但尚未完成的 I/O 操作——不等它们回来就销毁 ring，行为未定义。这个问题留到后续引入操作追踪机制后再处理。

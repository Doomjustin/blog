...
这篇延续前面 01-13 的实现主线，聚焦一个多用户聊天室的完整落地过程：
...
| **服务端架构** | 多 session 并发管理、消息广播、资源生命周期 |
| **客户端架构** | 键盘与网络的并发等待、双向通信、优雅退出 |
...
- 这样既能避免遍历容器时直接删元素，也便于统一通知“哪些客户端被移除”
...
- 两个任务**并发运行**（在同一个 IOContext 线程里交替执行）
...
2. **backpressure + 非阻塞入队**：容量约束配合 `try_send`，避免单个慢客户端拖慢整体广播

...
- 如何用 `Channel` 实现**协程间通信和 backpressure**
...
| **Channel 模式** | 如何用它解耦读写、提供 backpressure、避免死锁 |
...
- `broadcast()` 和 `announce()` 都是 async 的，会触发发送方的背压
...
1. **非阻塞读**：主协程在读取客户端输入时，不会被"写某个慢客户端"堵住
   ```
   场景：Client A 发来消息，需要转发给 B 和 C
   如果读写不分离：
     写给 B 卡住 → 整个 session 阻塞 → 无法读 A 的后续消息
   如果分离：
             ...
             Room& room) -> async::Task<>
{
    ...
    while (true) {
        ...
        if (chunk->data().empty()) break;  // EOF：连接关闭
        ...
        while (extract_line(pending, line)) {
            if (!line.empty()) {
                // 广播给其他客户端
                co_await room.broadcast(id, line + "\n");
            }
        }
    }
    ...
    log::info("[server] {} ({}) left the room", username, peer);

    // 9. 关闭出站队列并等待 writer 协程完成
    outbox.close();
    co_await group.join();
}
```
...
这样做保证：
- `write_loop` 在 `session` 结束前**必定完成发送**
...
**c) Channel 的 backpressure**

```cpp
co_await outbox.send(msg);  // 这一行可能阻塞
```

如果出站队列满了（容量 64），`send()` 会暂停当前协程，直到 `write_loop` 发送出一些消息腾出空间。
...
{
    co_await async::any(
        async::task(handle_terminal, std::ref(socket), name),
...
- `async::stop_then()` 包装所有 I/O 操作，使其可被 `async::any` 取消
- 当对方关闭时，`read()` 返回 EOF（字节数为 0）
- 空行被忽略（只发送有内容的消息）
...
- 返回时，**另一个任务被自动取消**
- 所有的 `co_await async::stop_then(...)` 都会立即返回，让任务快速退出
...
时刻 2：async::any 返回
  handle_terminal 被取消
  async::stop_then 中的 input.async_read() 立即返回异常
...
auto session_no_separation(socket) {
    while (true) {
        ...
        auto chunk = co_await stream.next();
        // 广播给所有客户端
        co_await room.broadcast(chunk);  // 这里可能很慢！
        // 如果某个客户端很慢，整个 read loop 被堵住
        // → 无法读取其他客户端的消息
    }
}
...
auto session_separated(socket) {
    ...
    while (true) {
        auto chunk = co_await stream.next();
        // 只是入队，不实际发送（异步）
        co_await room.broadcast(chunk);  // 这里快速返回
        // 即使有慢客户端，write_loop 会处理，read loop 继续
    }
}
```
...
auto read_loop(stream, room) {
    while (auto chunk = co_await stream.next()) {
        co_await room.broadcast(chunk);  // send() 内部会等待
    }
}
```
...
```
outbox 容量为 64
room.broadcast() 调用所有客户端的 outbox.send()

如果某客户端的 outbox 满了：
  send() 暂停 read_loop
  等待 write_loop 发送一些消息、腾出空间
  send() 恢复，继续
  
结果：read_loop 自动调节速度，防止堆积
```
...
| **Channel** | 协程间通信，提供 backpressure 和流控 |

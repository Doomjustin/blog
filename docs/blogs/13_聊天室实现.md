...
- `broadcast()` 和 `announce()` 使用 `async::timeout(..., 100ms)`，避免慢客户端无限拖慢广播
- 超时客户端会被关闭其 `outbox`，让系统优先保护整体吞吐与稳定性
...
```cpp
auto res = co_await async::timeout(outbox.send(msg), 100ms);
if (!res) {
    outbox.close();  // 慢客户端保护：超时后关闭该客户端通道
}
```
...
- 如果短时间内能入队，消息正常发送
- 如果超过 `100ms` 仍无法入队，判定该客户端过慢并关闭其通道
...
5. 主循环：持续读取消息并广播
   ├─ write_loop 并行运行，负责把消息发送出去
    ├─ 如果某客户端出站队列持续拥塞，`timeout(100ms)` 会触发隔离

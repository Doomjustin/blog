...
但是上文中留下一些坑又需要尽快填一下，否则后续再改会导致更大的重构。我们需要在这一篇里解释一下，为什么要在 `PollAwaiter` 中开放 set_result、prepare、context 这几个乍看上去毫无用处的接口。

先把现在的 `PollAwaiter` 完整展示一下——和 doc 02 里相比，多了这三个方法：
...
public:
    ...
    PollAwaiter(Context& context, int fd, short events)
      : context_{ context }, fd_{ fd }, events_{ events }
    {}
    ...
    // 新增：让 TimeoutAwaiter 能够直接组装 SQE
    void prepare(::io_uring_sqe* sqe) noexcept
    {
        ::io_uring_prep_poll_add(sqe, fd_, events_);
    }

    // 新增：让 TimeoutAwaiter 能够回写结果
    ...
    // 新增：让 TimeoutAwaiter 能够拿到上下文提交 SQE
...
在这个实现里，`std::expected` 把内核传回的 `-ETIME` 翻译成了标准的 `std::errc::timed_out`，这不是重点。更让我满意的，是 `timeout()` 作为一个独立包装器存在这件事本身。

最初我也考虑过给每个操作各加一个带超时的重载版本——`async_read_with_timeout`、`async_write_with_timeout` 之类。写了两个之后就意识到不对：这条路走下去，每加一个新操作，就要同时维护它的无超时版和有超时版，API 会越来越重，组合关系也会越来越乱。

`TimeoutAwaiter` 通过 Concept 只要求内层操作满足 `single_shot_only_operation`，跟具体是什么操作无关，所以它能叠在任何满足条件的操作上。之后加新的 I/O 操作，超时能力自动跟上，不需要单独维护。

...
错误处理上，我的判断是把两类情况分开对待。`socket` 创建、`bind`、`setsockopt` 这些初始化步骤，失败就直接抛异常——出了问题通常是配置错了或者资源耗尽，继续跑下去也没意义。但 `read`、`write`、`accept` 这类 I/O 操作不一样，超时、对端断开这些都是正常的运行时分支，不该用异常来表达，所以这部分统一返回 `std::expected<T, std::error_code>`。
...
buffer 参数直接用 `std::span<std::byte>` 取代了 C 风格的 `void* + size_t`，字节流的意图更清楚，指针越界的风险也在编译期就消掉了。
...
`AcceptAwaiter` 负责处理被动连接接入。在原生 C 接口中，`accept` 可以通过传入 `sockaddr*` 来获取对端地址信息。由于当前版本的实现并未对地址（Endpoint）进行体系化封装，该版本的 `AcceptAwaiter` 暂不支持提取连接地址，只是把内核返回的新 `fd` 直接包装成对应的 `Socket` 实例交出去。
